...
Cuando se muestra el gráfico por primera vez, la pestaña «1 semana» aparece preseleccionada y presenta los datos correspondientes a una semana en formato gráfico para que sea más fácil entender grandes cantidades de datos y la relación entre las diferentes series de datos. Si quieres restablecer el intervalo de fechas, por ejemplo, puedes seleccionar la pestaña «1 mes» y elegir las fechas en el calendario.
...
.Pasos
. En el panel de control, selecciona *Rendimiento*.
+
En el caso de los sistemas ONTAP, puedes seleccionar la pestaña *Nodo* para ver el rendimiento de un único nodo de un clúster ONTAP, seleccionar la pestaña *Nivel local* para ver el rendimiento del nivel local, o seleccionar la pestaña *Volumen* para ver el rendimiento del volumen. De forma predeterminada, se muestra el rendimiento del clúster.
