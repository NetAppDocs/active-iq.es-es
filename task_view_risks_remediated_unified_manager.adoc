---
sidebar: sidebar 
permalink: task_view_risks_remediated_unified_manager.html 
keywords: risks, actions, remediation, corrective action, unified manager 
summary: Proporciona información para ver los riesgos que se pueden solucionar automáticamente mediante Unified Manager. 
---
= Visualice los riesgos de las acciones en Digital Advisor y mitigue utilizando Unified Manager o Ansible Playbook
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Puede analizar su sistema de almacenamiento controlando las acciones y riesgos y mitigarlo con Active IQ Unified Manager o el libro de aplicaciones de Ansible.

video::XusFvXM7h-E[youtube,width=848,height=480]
.Pasos
. Selecciona *Ver todas las acciones* en el panel de control.
. Selecciona *Acciones* para ver los riesgos asociados a las acciones, selecciona *Riesgos* para ver todos los riesgos o selecciona *Sistemas afectados* para ver los sistemas que requieren atención.
+
Si el riesgo se puede mitigar con Active IQ Unified Manager, el image:spanner.png["Llave inglesa"] El icono se resalta y, si el riesgo se puede mitigar con el libro de aplicaciones de Ansible, el image:update_ansible.png["Actualice Ansible"] icono resaltado.



[cols="50,50"]
|===
| Para mitigar el riesgo con Unified Manager | Para mitigar los riesgos con el libro de aplicaciones de Ansible 


 a| 
. Selecciona el icono image:spanner.png["Llave inglesa"].
. Selecciona *Fix It* para iniciar Active IQ Unified Manager.
. Selecciona *Instalar* para instalar Active IQ Unified Manager 9.7 o una versión posterior para usar la opción *Fix It*.
. Selecciona *Actualizar* para actualizar a Active IQ Unified Manager 9.7 o una versión posterior para usar la opción *Fix It*.

 a| 
. Selecciona el icono image:update_ansible.png["Actualice Ansible"].
. Selecciona *Descargar* para descargar el paquete de Ansible Automation con el firmware de AFF y FAS.


|===

NOTE: Se requiere un contrato de SupportEdge Advisor o SupportEdge Expert para utilizar la opción *Fix it* y las características del libro de aplicaciones de Ansible.
